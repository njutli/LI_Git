...
### 元数据
fs_hdr: 磁电盘上存储的文件系统元数据，在格式化的时候设置，包括magic，盘框号等信息；
hdr_xx: 单个镜像的元数据，包括magic，归档镜像编号，meta/data的大小等信息；
obj文件: 记录镜像编号与磁电盘位置的对应关系

### 崩溃一致性
1、SSD上文件
通过tmp目录实现，崩溃后发现tmp目录上的文件不完整，直接删除

2、归档流程中的文件
通过wal记录当前写入的container编号，崩溃重启后继续写磁电盘

归档镜像大小： 88G；根据磁电盘寿命要求计算，太小会影响磁电盘写入次数，即电机启停次数，太大对SSD有更高要求
