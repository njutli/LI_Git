...
美团mysql问题定位
```
1、先用blk-throtl和cgroup限速确认IO是否是关键路径
2、查看盘配置（IO确认是关键路径）
find /sys/block/sda/ -type f | xargs grep .
查看磁盘配置并对比
3、blktrace抓取IO耗时情况（IO确认是关键路径，且配置无差异）
4、（耗时在D2C）
D2C阶段的耗时主要在IO下发到中断返回后CPU开始处理，但这段时间又包括IO下发到设备返回中断和设备返回中断到CPU开始处理，暂时无法确认是哪段耗时较长。如果是IO下发到设备返回中断这段时间长，可以认为是设备差异导致；如果是设备返回中断到CPU开始处理时间长，则可以认为是内核差异导致。且当前存在处理IO中断的CPU同时处理大量网络中断的情况，不能排除网络中断导致的IO性能波动。

建议做下面几个试验：
1、jbd2绑核
将一个CPU隔离出来，单独给jbd2使用，保证不会出现IO和其他中断共享CPU的情况。
昨天做了jbd2绑核的动作，但不能确认还有没有共享CPU的情况，建议在jbd2绑核测试的情况下用脚本抓一下，看下还有没有共享CPU的情况（脚本我再更新一下，对IO中断的记录增加进程信息）
2、使用SP4版本验证
基于SP4版本，开关IO异步下发特性，确认是否存在差异
3、使用4.18环境验5.10
排查设备个体差异导致的影响


从最新反馈的数据看，不同的jbd2进程是否共享CPU对性能并没有影响，目前只有怀疑nvme驱动在两个版本间的差异了。
后续建议：
1、两个版本使用fio测试对比，尝试简化会导致差异的IO模型
-iodepth=8
-rw=randrw
-numjobs=8
-bs=16k
-rwmixread=33
使用fio测试，看是否能复现出两个版本的性能差异，测试过程抓blktrace和iostat
2、提供两个版本的config
可能和nvme驱动相关的配置有关，麻烦提供下两个版本的config让我们对比下
3、二分查找差异点
目前没有更多的定位手段了，如果上述两步都找不到原因，估计只能通过二分查找导致性能差异的补丁了

这是之前定位一个性能问题的思路和过程，看看有没有什么需要补充的
```


```
D2C延时高：设备侧返回中断慢；内核处理中断慢；通过绑核确认，排除其他中断的影响
Q2D延时高：
  Q2G延时高：IO压力大，tag不够（bpftrace抓拿tag延时？）；throttle策略不合理，bio阻塞（查看各策略参数）
  G2I延时高：IO合并？锁争用？
  I2D延时高：cgroup限速？
...
# 什么情况下会出发优先级反转，该怎么防止这种情况
例子（假设任务优先级：T1 > T2 > T3）：
1. T3（低优先级） 先运行，获取了互斥锁 M。
2. T1（高优先级） 就绪，抢占 T3，开始运行。
3. T1 尝试获取互斥锁 M，但 M 被 T3 持有，于是 T1 被阻塞，等待 M。
4. 此时 CPU 会调度就绪状态中优先级最高的任务，于是 T2（中优先级） 获得 CPU，开始运行。
5. T2 不涉及互斥锁 M，可以一直运行（甚至可能长时间占用 CPU），于是 T3 无法继续执行，也就无法释放 M。
6. 结果：T1 等待 T3 释放 M，但 T3 被 T2 抢占无法运行，导致 T1 被一个优先级低于 T2 的任务间接阻塞，这就是优先级反转。

优先级继承：
当高优先级任务因请求锁而被低优先级任务阻塞时，​低优先级任务临时继承高优先级任务的优先级，直到释放锁
...
1、内存直接回收和间接回收的区别是什么？
2、块层 tag 的作用是什么？
3、blktrace 是如何统计统计各个阶段的时间的？
4、io_util 的含义？100% 表示满载了吗？

第一模块：优势深度挖掘（你自己的优势准备，这里以“文件系统与IO”为例）
问题会非常深入源码和机制关联。
核心原理：
“请详细描述 ext4文件系统 在处理一个随机小文件写入时，从VFS层到块设备层的完整IO路径。”
“ext4的日志（JBD2） 是如何工作的？它如何保证数据一致性？如果禁用日志，可能带来什么风险和性能变化？”
“请谈谈 Page Cache、Buffer Cache 以及 磁盘的读写缓存 之间的关系。内核的 缓存回写 线程是如何触发的？有哪些主要策略？”
“当系统内存不足时，内存回收 过程是如何与文件系统IO相互影响的？pdflush 或 writeback 机制在这里扮演什么角色？”
如果有社区贡献：
5. “你提交的内核Patch具体是针对哪个子系统或文件的？你发现这个问题的契机是什么？在解决过程中，除了你采用的方案，是否考虑过其他备选方案？”
dm设备初始化过程中下发IO，访问特定未完成初始化的资源导致空指针解引用。
方案一：阻塞IO，等设备初始化完成再下发
方案二：IO直接失败
  社区先采用方案二，又改成方案一，个人认可方案二

第二模块：实战能力验证（故障定位）
问题会模拟真实线上场景。
场景排查：
“假设线上服务器监控到 await（IO等待时间）指标异常飙高，但磁盘使用率并不高。你的排查思路是什么？会依次使用哪些工具（如 iostat, blktrace, bcc工具等）？可能的原因有哪些？”
“如果业务方报告文件读取性能突然下降，你会如何判断是Page Cache命中率问题，还是底层磁盘本身的问题？”

第三模块：知识广度探查（内存、调度及其他）
问题会触及候选人的知识边界。
短板探查：
（关于内存管理）“谈谈Linux的 物理内存管理（伙伴系统） 和 虚拟内存管理（MMU，页表） 的基本原理。vm.swappiness 参数的作用是什么？”
（关于任务调度）“CFS调度器 的核心思想是什么？vruntime 是如何计算的？nice值如何影响进程优先级？”
（关于工具原理）“你常用 perf 或 ftrace 来定位问题，能否说明一下 perf 的采样原理 以及 ftrace 的tracepoint和kprobe的主要区别？”
```
