...
## （二）磁电盘
```
企业数据快速增长，离线介质成为降成本、绿色节能优选方案。华为推出首款72TB磁电盘，用于替代机械硬盘。SSD与磁带系统组成的MED存储设备需要定制的文件系统对数据进行管理
...
主导文件堆叠方案设计，mkfs/mount多盘支持等方案设计；
完成挂载数量限制，mkfs/mount基础功能支持等功能点实现，累积开发代码3k+；
完成目录压缩、mkfs/mount基础功能验证等测试用例设计，累积开发代码2k+；
定位spinfs及erofs问题，bugfix补丁累计20+。
负责图形环境的搭建与调试
